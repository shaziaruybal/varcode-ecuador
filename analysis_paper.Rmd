...
### Load data
```{r}
ups <- fread(here::here("data", "ecuador_ups_classification_final.csv"), data.table = F, )
binary_all <- fread(here::here("data", "ecuador_SAm_binary_matrix_final.csv"), data.table = F)
binary_ecu <- fread(here::here("data", "ecuador_binary_matrix_final.csv"), data.table = F)
microsat_ecu <- fread(here::here("data", "ecuador_binary_microsat_final.csv"), data.table = F)
ecu_epi <- fread(here::here("data", "ecuador_epi.csv"), data.table = F)
outbreak_metadata <- read.csv(here::here("data", "ecuador_varcode1_47types_metadata.csv"))
```
...
```{r}
ecu_epi %>% filter(OutbreakBinary == "NotOutbreak2013", Year>2013, !is.na(Age)) %>% 
  ggplot(aes(x = Sex, y = Age)) + geom_boxplot() + geom_point() + stat_compare_means(method = "wilcox.test", paired = F)
...
```{r}
ecu_epi %>% filter(OutbreakBinary != "Outbreak2013", Year > 2013, !is.na(Age) & !is.na(Sex)) %>% group_by(OutbreakRecombinant) %>% tally()
ecu_epi %>% filter(OutbreakBinary != "Outbreak2013", Year > 2013, !is.na(Age) & !is.na(Sex)) %>% group_by(OutbreakRecombinant, Sex) %>% tally()
...
## For revision
```{r}
ecu_epi %>% filter(OutbreakBinary != "Outbreak2013", Year > 2013, !is.na(Age)) %>% group_by(OutbreakRecombinant) %>% dplyr::summarize(min = min(Age), med = median(Age), avg = mean(Age), max = max(Age))

ecu_epi %>% filter(OutbreakBinary != "Outbreak2013", Year > 2013, !is.na(Age)) %>% 
  ggplot(aes(x = OutbreakRecombinant, y = Age)) + geom_boxplot() + geom_point() + stat_compare_means(method = "kruskal.test", paired = F)
```

```{r}
ecu_epi %>% tabyl(OutbreakRecombinant)
ecu_epi %>% filter(Year > 2013) %>% tabyl(OutbreakRecombinant)
ecu_epi %>% filter(Year > 2013) %>% tabyl(OutbreakRecombinant, Year) 
ecu_epi %>% filter(Year > 2013) %>% tabyl(OutbreakRecombinant, Year) %>% adorn_percentages("col") %>% adorn_pct_formatting(1)
```
